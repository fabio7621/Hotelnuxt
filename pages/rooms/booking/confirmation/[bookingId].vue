...
const { data: bookingResultList } = await useFetch(`api/v1/orders/${bookingId}`, {
  baseURL: apiUrl,
  headers: {
      ...
      <div class="row gap-15 gap-md-0">
        <div class="col-12 col-md-7">
          <div class="d-flex flex-column flex-md-row align-items-md-center gap-4 gap-md-10 mb-8 mb-md-10">
            ...
            <div class="text-neutral-0 fs-1">
              <h1 class="fw-bold">恭喜，{{ userName }}！</h1>
          ...
          <div class="d-flex flex-column align-items-md-start">
            ...
            <NuxtLink to="/user/order" class="btn btn-primary-100 px-md-15 py-4 text-neutral-0 fw-bold border-0 rounded-3" type="button">前往我的訂單</NuxtLink>
          </div>
          ...
          <div class="d-flex flex-column gap-6">
            <div>
              ...
              <span class="text-neutral-0 fw-bold">{{ bookingResultList.userInfo.name }}</span>
            </div>
            <div>
              <p class="mb-2 text-neutral-40 fw-medium">手機號碼</p>
              <span class="text-neutral-0 fw-bold">{{ bookingResultList.userInfo.phone }}</span>
            </div>
            <div>
              <p class="mb-2 text-neutral-40 fw-medium">電子信箱</p>
              <span class="text-neutral-0 fw-bold">{{ bookingResultList.userInfo.email }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="col-12 col-md-5">
          <div class="rounded-3xl d-flex flex-column gap-6 gap-md-10 p-4 p-md-10 mx-auto ms-md-auto bg-neutral-0" style="max-width: 478px">
            ...
            <img class="img-fluid rounded-3" :src="bookingResultList.roomId.imageUrl" :alt="bookingResultList.roomId.name" />

            <section class="d-flex flex-column gap-6">
              <h3 class="d-flex align-items-center mb-6 text-neutral-80 fs-8 fs-md-6 fw-bold">
                <p class="mb-0"></p>
                <p class="mb-0">{{ bookingResultList.roomId.name }}，{{ countDateDiffs(bookingResultList.checkInDate, bookingResultList.checkOutDate) }} 晚</p>
                <span class="d-inline-block mx-4 bg-neutral-80" style="width: 1px; height: 18px" />
                <p class="mb-0">住宿人數：{{ bookingResultList.peopleNum }} 位</p>
              </h3>

              <div class="text-neutral-80 fs-8 fs-md-7 fw-bold">
                <p class="title-deco mb-2">入住：<span v-timeFormat="bookingResultList.checkInDate"></span>，15:00 可入住</p>
                <p class="title-deco mb-0">退房：<span v-timeFormat="bookingResultList.checkOutDate"></span>，12:00 前退房</p>
              </div>

              <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                NT$ {{ $formatPrice(bookingResultList.roomId.price * countDateDiffs(bookingResultList.checkInDate, bookingResultList.checkOutDate)) }}
              </p>
            </section>
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <li class="flex-item d-flex gap-2" v-for="facility in bookingResultList.roomId.facilityInfo" :key="facility.title">
                  <Icon
                    class="fs-5"
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <li class="flex-item d-flex gap-2" v-for="amenity in bookingResultList.roomId.amenityInfo" :key="amenity.title">
                  <Icon
                    class="fs-5"

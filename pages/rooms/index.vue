...
const roomList = ref([]);
const { data: rooms, error: roomError } = await useFetch(`api/v1/rooms/`, {
  baseURL: apiUrl,
});
onMounted(() => {
  if (rooms.value?.result) {
    ...
    <section class="py-10 py-md-30 bg-primary-10">
      <div class="container mb-md-12">
        ...
        <ul class="d-flex flex-column gap-6 gap-md-12 list-unstyled">
          <li v-for="item in roomList" :key="item._id" class="card flex-lg-row border-0 rounded-3xl overflow-hidden">
            <div class="row">
              <div class="col-12 col-lg-7">
                ...
                >
                  <swiper-slide v-for="(itemPic, index) in item.imageUrlList" :key="index">
                    <picture>
                      <source :srcset="itemPic" />
                      <img class="w-100 object-fit-cover" :src="itemPic" loading="lazy" :alt="item.name" />
                    </picture>
                  </swiper-slide>
                </swiper>
              </div>
              <div class="col-12 col-lg-5">
                <div class="card-body pe-md-10 py-md-10">
                  <h3 class="card-title fs-2 fw-bold text-neutral-100">{{ item.name }}</h3>
                  ...
                  <ul class="d-flex gap-4 mb-6 mb-md-10 list-unstyled">
                    <li class="card-info px-4 py-5 border border-primary-40 rounded-3">
                      ...
                      <p class="mb-0 fw-bold text-neutral-80 text-nowrap">{{ item.areaInfo }}</p>
                    </li>
                    <li class="card-info px-4 py-5 border border-primary-40 rounded-3">
                      <Icon class="mb-2 fs-5 text-primary-100" icon="material-symbols:king-bed" />
                      <p class="mb-0 fw-bold text-neutral-80 text-nowrap">{{ item.bedInfo }}</p>
                    </li>
                    <li class="card-info px-4 py-5 border border-primary-40 rounded-3">
                      <Icon class="mb-2 fs-5 text-primary-100" icon="ic:baseline-person" />
                      <p class="mb-0 fw-bold text-neutral-80 text-nowrap">2-{{ item.maxPeople }} 人</p>
                    </li>
                  </ul>
                  ...
                  <div class="d-flex justify-content-between align-items-center fs-7 fs-md-5 text-primary-100">
                    ...
                    <NuxtLink :to="`/rooms/${item._id}`" class="icon-link icon-link-hover text-primary-100">
